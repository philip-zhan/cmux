...
- **Wave D — Command catalog + capability scoping + typed params.** Generate the full `cmux capabilities` catalog into authoring docs + the `cmux-swift-interpreter` skill; add capability scoping (default-deny dangerous namespaces for untrusted sidebars); robust param coercion. Security gate for "all commands exposed."
